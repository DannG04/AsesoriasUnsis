...
public interface AsesoriaRepository extends JpaRepository<Asesoria, Integer> {
    ...
    @Query(value = "SELECT registrar_asesoria(" +
            ":comentario, " +
            "CAST(:lugarAsesoria AS \"tipo_sustitucion\"), " +
            "CAST(:idProfesor AS \"idValida\"), " +
            "CAST(:matricula AS \"matriculaValida\"), " +
            ":materiaSeleccionada, " +
            "CAST(:fecha AS DATE), " +
            "CAST(:horaInicio AS TIME), " +
            "CAST(:horaFinal AS TIME))", nativeQuery = true)
